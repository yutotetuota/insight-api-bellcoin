...
var bitcoinDataDirs = ['/tmp/bellcoin1', '/tmp/bellcoin2'];
...
var startBitcoind = function(count, callback) {
  ...
  async.timesSeries(count, function(n, next) {
    ...
    rimraf(datadir, function(err) {
      ...
      console.log(count + ' bellcoind\'s started at pid(s): ' + pids);
      callback();
  });
};
...
var buildInitialChain = function(callback) {
  async.waterfall([
    function(next) {
      console.log('checking to see if bellcoind\'s are connected to each other.');
  ...
  it('should get status: /status', function(done) {
    ...
    var request = http.request('http://localhost:53001/api/status', function(res) {
      ...
      if (res.statusCode !== 200 && res.statusCode !== 201) {
        ...
        return done('Error from bellcore-node webserver: ' + res.statusCode);
